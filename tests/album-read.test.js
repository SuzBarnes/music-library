...
describe('read album', () => {
    ...
    const createdArtists = await Promise.all ([
        ...
        //    console.log(createdArtists);
           const createdArtistsIds = createdArtists.map(artist => {return artist[0].insertId});
    ...
    const createdAlbums = await Promise.all([
        ...
        const createdAlbumsIds = createdAlbums.map(album => {return album[0].insertId});
...
//             const res = await request(app).get(`/artist/:artistId/album/${expected.id}`).send();
...
//             const res = await request(app).get('/artist/:artistId/album/999999').send();
